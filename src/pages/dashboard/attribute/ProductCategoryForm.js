...
export default function ProductCategoryForm({ productType, currentValue }) {
  ...
  const onSubmit = async (data) => {
    await axiosInstance.put(`/productcategory/${currentValue._id}`, data)
      .then((res) => {
  ...
  const onRemove = async () => {
    await axiosInstance.delete(`/productcategory/${currentValue._id}`)
      .then((res) => {
  ...
  const onCreate = async (data) => {
    await axiosInstance.post('/productcategory', data)
      .then((res) => {
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Card sx={{ p: 3 }}>
        <Stack spacing={3}>
          ...
          >
            {
              productType.map((item) => (<option key={item._id} value={item.name}>
                {item.name}
              </option>
              ))
            }
