...
export default function EcommerceProductList() {
  ...
  const handleSelectAllClick = (checked) => {
    if (checked) {
      const selected = productList.map((n) => n.prod_name);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const isItemSelected = selected.indexOf(row.prod_name) !== -1;
                    return (
                      <TableRow
                        hover
                        key={row._id}
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(row.prod_name)} />
                        </TableCell>
                        <TableCell sx={{ display: 'flex', alignItems: 'center' }}>
                          <Image
                            disabledEffect
                            alt={row.prod_name}
                            src={row.avatar_photo_prod}
                        ...
                        <TableCell style={{ minWidth: 160 }}>{fDate(row.created_at)}</TableCell>
                        <TableCell align="right">{fCurrency(row.prod_price)}</TableCell>
                        <TableCell align="right">
                          <ProductMoreMenu productName={row.prod_name} onDelete={() => handleDeleteProduct(row._id)} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
